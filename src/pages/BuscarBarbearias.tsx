  ...
  return <div className="min-h-screen bg-background p-3 sm:p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-6 sm:mb-8 animate-fade-in px-2">
          <img src={logo} alt="Corte & Arte" className="h-10 sm:h-12 w-auto mx-auto mb-3 sm:mb-4" />
          <h1 className="text-xl sm:text-2xl font-semibold text-foreground mb-2">
            Descubra a barbearia ideal
          </h1>
        ...
        {/* Geolocalização */}
        {!localizacaoPermitida && <div className="mb-4 sm:mb-6 mx-2 sm:mx-0">
            <Card className="border-2 border-dashed border-primary/20 animate-scale-in">
              <CardContent className="p-4 sm:p-6 text-center">
                ...
                <h3 className="font-medium mb-2 text-sm sm:text-base">Encontrar estabelecimentos próximos a você?</h3>
                <p className="text-xs sm:text-sm text-muted-foreground mb-4 px-2">
                  Identifique salões disponíveis na sua região
                </p>
                <Button onClick={solicitarLocalizacao} disabled={carregandoLocalizacao} className="w-full sm:w-auto text-sm" size="sm">
        ...
        <div className="mb-6 sm:mb-8 mx-2 sm:mx-0">
          <div className="flex items-center gap-2 mb-3 sm:mb-4 px-1">
            ...
            <h2 className="text-base sm:text-lg font-semibold">Melhores Salões</h2>
          </div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">
            {topBarbearias.map((barbearia, index) => <Card key={barbearia.id} className="hover:shadow-lg transition-all duration-300 animate-scale-in border-primary/10 mx-1 sm:mx-0">
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center gap-2 sm:gap-3 mb-3">
                    <div className="relative">
                      {barbearia.logo_url ? <img src={barbearia.logo_url} alt={`Logo da ${barbearia.name}`} className="h-10 w-10 sm:h-12 sm:w-12 rounded-full object-cover border-2 border-primary/20" /> : <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center">
                      ...
                      {index === 0 && <Crown className="absolute -top-2 -left-2 h-3 w-3 sm:h-4 sm:w-4 text-yellow-500" />}
                    </div>
        ...
        <div className="mb-6 sm:mb-8 mx-2 sm:mx-0">
          <Card>
            <CardHeader className="pb-3 sm:pb-6">
              <CardTitle className="flex items-center gap-2 text-base sm:text-lg">Buscar Estabelecimento<Search className="h-4 sm:h-5 w-4 sm:w-5" />
            ...
            <CardContent className="pt-0">
              <div className="space-y-4 sm:grid sm:grid-cols-1 sm:space-y-0 md:grid-cols-2 lg:grid-cols-3 sm:gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="estado" placeholder="Ex: SP, RJ, MG..." value={estado} onChange={e => setEstado(e.target.value.toUpperCase())} maxLength={2} className="text-sm" />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="cidade" className="text-sm">Cidade</Label>
                  <Input id="cidade" placeholder="Ex: São Paulo, Jandira..." value={cidade} onChange={e => setCidade(e.target.value)} className="text-sm" />
                </div>
        ...
        <div className="space-y-4 mx-2 sm:mx-0">
          ...
          {resultados.length === 0 ? <Card>
            ...
            </Card> : loading ? <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            </div> : <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {resultados.map(barbearia => <Card key={barbearia.id} className="hover:shadow-lg transition-all duration-300 animate-fade-in group">
                  <CardContent className="p-4 sm:p-6">
                    <div className="text-center space-y-3 sm:space-y-4">
                      <div className="relative">
                        {barbearia.logo_url ? <img src={barbearia.logo_url} alt={`Logo da ${barbearia.name}`} className="h-12 w-12 sm:h-16 sm:w-16 mx-auto rounded-full object-cover border-2 border-primary/20 group-hover:scale-110 transition-transform duration-300" /> : <div className="h-12 w-12 sm:h-16 sm:w-16 mx-auto rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                            <span className="text-lg sm:text-2xl font-bold text-primary">
                              {barbearia.name?.charAt(0) || 'B'}
                            </span>
                          </div>}
                        {barbearia.likes_count >= 5000 && <div className="absolute -top-2 -left-2 sm:-top-3 sm:-left-3 bg-yellow-500 text-white text-xs px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full font-medium flex items-center gap-1">
                      ...
                      <div className="space-y-2">
                        ...
                        {(barbearia.city || barbearia.state) && <div className="flex items-center justify-center gap-1 text-muted-foreground text-xs sm:text-sm">
                            <MapPin className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                            <span className="truncate">
                              {barbearia.city && barbearia.state ? `${barbearia.city}, ${barbearia.state}` : barbearia.city || barbearia.state || 'Localização não informada'}
                            </span>
                          </div>}
                        ...
                        <div className="flex justify-center py-2">
                          {barbearia.id && barbearia.id.length > 0 && <LikeButton key={`like-${barbearia.id}`} targetType="company" targetId={barbearia.id} />}
                        </div>
                      </div>
