...
// Funções para salvar/carregar cliente
  ...
  useEffect(() => {
    ...
    const loadClientData = async () => {
      ...
      if (saved && mounted) {
        setFormData(prev => ({
          ...prev,
          nome: saved.nome || prev.nome,
          telefone: saved.telefone || prev.telefone,
      ...
      try {
        ...
        console.log('Sem dados do usuário ou erro ao buscar.');
      }
    };
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <Label>Serviço</Label>
                  {services.length === 0 ? <div>Carregando serviços...</div> : (
                ...
                <div>
                  ...
                  {professionals.length === 0 ? <div>Carregando profissionais...</div> : (
                ...
                <div>
                  ...
                  <Select onValueChange={(v) => handleInputChange('data', v)}>
                    ...
                    <SelectContent>
                      {diasDisponiveis.map(d => <SelectItem key={d.data} value={d.data}>{d.texto}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <Label>Horário</Label>
                  {loadingSlots ? <div>Buscando horários...</div> :
                    availableSlots.length === 0 ? <div>Nenhum horário disponível</div> :
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div>
                    <Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Enviando...' : 'Confirmar Agendamento'}</Button>
                  </div>
                </div>
              </form>
            </CardContent>
